...
class Login{
    ...
    #GetUserFromDatabase(username_email,plainTextPassword,res,next){
        ...
        db.query(sql,[username_email,username_email],(err,rows,fields) => {
            if(err) throw err
            if(rows.length > 0){
                this.#PasswordCompare(plainTextPassword,rows[0].password,rows[0].iduser,rows[0].email,rows[0].username,res,next)
            }else{
    ...
    #PasswordCompare(plainTextPass,hashedPassword,iduser,email,username,res,next){
